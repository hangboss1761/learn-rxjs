...
const randomLetter = () =>
  String.fromCharCode(Math.random() * ('z'.charCodeAt(0) - 'a'.charCodeAt(0)) + 'a'.charCodeAt(0));
...
const renderGame = (state: State) => (
  ...
  state.letters.forEach((l) => (document.body.innerHTML += <string>'&nbsp'.repeat(l.yPos) + l.letter + '<br/>')),
